...
<header class="sticky top-0 z-40 bg-white/70 backdrop-blur-md border-b border-cyan-100/50 px-4 sm:px-6 py-3 shadow-md">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex items-center flex-wrap justify-end gap-3 sm:gap-4">
            ...
            {{-- Messages --}}
            <a href="{{route('admin.chats.index')}}" class="relative text-gray-500 hover:text-emerald-500">
                <i class="fas fa-envelope text-sm"></i>
                <span id="chatUnreadBadge" class="absolute -top-1 -right-1 min-w-4 h-4 px-1 bg-cyan-500 text-white text-[8px] flex items-center justify-center rounded-full hidden"></span>
            </a>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        async function refreshChatBadge() {
            ...
            try {
                ...
                const unreadCount = data.data.reduce((sum, chat) => sum + Number(chat.unread_count || 0), 0);
