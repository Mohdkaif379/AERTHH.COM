...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-xl border border-cyan-100/50 shadow-sm overflow-hidden">
        <div class="px-6 py-4 bg-gradient-to-r from-cyan-50 to-emerald-50 border-b border-cyan-100/50">
            <h3 class="text-sm font-semibold text-gray-700">Add New Attribute</h3>
            <p class="text-[10px] text-gray-500 mt-1">Create attributes like Size, Color, Material, etc.</p>
        </div>

        <form action="{{ route('attribute.store') }}" method="POST" class="p-6 space-y-5">
            @csrf
            ...
            <div>
                ...
                @enderror
                <p class="mt-1 text-[10px] text-gray-400">This will be used as a product attribute filter</p>
            </div>

            {{-- Preview Card --}}
            <div class="bg-cyan-50/30 rounded-lg p-4 border border-cyan-100">
                <h4 class="text-xs font-medium text-cyan-700 mb-2 flex items-center">
                    <i class="fas fa-info-circle mr-1"></i>
                    Attribute Information
                </h4>
                <ul class="text-[10px] text-gray-500 space-y-1 list-disc list-inside">
                    <li>Attribute name must be unique</li>
                    <li>Examples: Size, Color, Material, Weight, Dimensions</li>
                    <li>You can add attribute values after creating the attribute</li>
                </ul>
            </div>
