...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-xl border border-cyan-100/50 shadow-sm overflow-hidden">
        <div class="px-6 py-4 bg-gradient-to-r from-cyan-50 to-emerald-50 border-b border-cyan-100/50">
            <h3 class="text-sm font-semibold text-gray-700">Edit Brand</h3>
        </div>

        <form action="{{ route('brand.update', $brand->id) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-5">
            @csrf
            ...
            <div>
                ...
                <input type="text" 
                       name="alt_text"
                       value="{{ old('alt_text', $brand->alt_text) }}"
            ...
            <div>
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <span class="text-xs text-gray-500">{{ $brand->image }}</span>
                </div>
            </div>
            @endif
            ...
            <div>
                <label class="block text-xs font-medium text-gray-600 mb-1.5">
                    Update Image <span class="text-gray-400">(Leave empty to keep current)</span>
                </label>
                <div class="flex items-center space-x-4">
                    ...
                    <div class="relative w-20 h-20 bg-gray-100 rounded-lg border-2 border-dashed border-gray-300 flex items-center justify-center overflow-hidden group">
                        {{-- Default Icon --}}
                        <i class="fas fa-image text-gray-400 text-2xl preview-default {{ $brand->image ? 'hidden' : '' }}"></i>
