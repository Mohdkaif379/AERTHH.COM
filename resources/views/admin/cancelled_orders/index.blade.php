...
<div class="space-y-6">
    <div class="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
            ...
            <p class="text-[10px] text-gray-400">Browse all orders with <span class="font-medium text-red-600">cancelled</span> status</p>
        </div>
        ...
        <div class="w-full lg:w-[360px]">
            <div class="relative">
                ...
                <input
                    type="text"
                    id="cancelledOrderSearch"
                    placeholder="Search order, customer, product..."
                    class="w-full pl-8 pr-3 py-2 bg-white border border-gray-200 rounded-lg focus:border-red-300 focus:outline-none focus:ring-1 focus:ring-red-100 text-xs"
                >
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-red-100 shadow-sm overflow-hidden">
        <div class="px-4 sm:px-6 py-4 bg-gradient-to-r from-red-50 to-rose-50 border-b border-red-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        ...
        @if($orders->count() > 0)
            <div class="md:hidden divide-y divide-gray-100" id="cancelledOrderMobileList">
                @foreach($orders as $index => $order)
                    ...
                    @endphp
                    <div class="p-4 space-y-3 cancelled-order-card" data-search="{{ strtolower($order->order_no.' '.$customerName.' '.$productName.' '.$vendorName.' '.$order->status) }}">
                        <div class="flex items-start justify-between gap-3">
                            ...
                            <span class="px-2 py-1 rounded-full text-[10px] font-bold bg-red-50 text-red-700 border border-red-100 uppercase tracking-tighter shadow-sm">
                                Cancelled
                            </span>
                        </div>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @foreach($orders as $index => $order)
                            @php
                                ...
                                $vendorName = $order->vendor->name ?? 'Unknown Vendor';
                            @endphp
                            <tr class="hover:bg-red-50/30 transition cancelled-order-row" data-search="{{ strtolower($order->order_no.' '.$customerName.' '.$productName.' '.$vendorName.' '.$order->status) }}">
            ...
            <div id="cancelledOrderNoResult" class="hidden px-6 py-16 text-center text-gray-400">
                <i class="fas fa-search text-3xl mb-3 block text-gray-300"></i>
                No matching cancelled orders found
            </div>

            <div class="px-4 sm:px-6 py-4 border-t border-gray-100" id="cancelledOrderPagination">
                {{ $orders->links() }}
            </div>
        @else
            <div class="px-6 py-16 text-center text-gray-400" id="cancelledOrderEmptyState">
                <i class="fas fa-times-circle text-3xl mb-3 block text-gray-300"></i>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        const searchInput = document.getElementById('cancelledOrderSearch');
        if (!searchInput) return;

        const rows = Array.from(document.querySelectorAll('.cancelled-order-row'));
        const cards = Array.from(document.querySelectorAll('.cancelled-order-card'));
        const pagination = document.getElementById('cancelledOrderPagination');
        const noResultState = document.getElementById('cancelledOrderNoResult');
