...
<div class="space-y-6">
    <div class="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
            ...
            <p class="text-[10px] text-gray-400">Browse all orders that <span class="font-medium text-rose-600">failed to reach</span> customers</p>
        </div>
        ...
        <div class="w-full lg:w-[360px]">
            <div class="relative">
                ...
                <input
                    type="text"
                    id="failedOrderSearch"
                    placeholder="Search order, customer, product..."
                    class="w-full pl-8 pr-3 py-2 bg-white border border-gray-200 rounded-lg focus:border-rose-300 focus:outline-none focus:ring-1 focus:ring-rose-100 text-xs"
                >
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-rose-100 shadow-sm overflow-hidden">
        <div class="px-4 sm:px-6 py-4 bg-gradient-to-r from-rose-50 to-pink-50 border-b border-rose-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <h3 class="text-sm font-semibold text-gray-700 flex items-center space-x-2">
                <i class="fas fa-list text-rose-500 text-sm"></i>
                <span>Failed Delivery List</span>
            </h3>
        ...
        @if($orders->count() > 0)
            <div class="md:hidden divide-y divide-gray-100" id="failedOrderMobileList">
                @foreach($orders as $index => $order)
                    ...
                    @endphp
                    <div class="p-4 space-y-3 failed-order-card" data-search="{{ strtolower($order->order_no.' '.$customerName.' '.$productName.' '.$vendorName.' '.$order->status) }}">
                        <div class="flex items-start justify-between gap-3">
                            ...
                            <span class="px-2 py-1 rounded-full text-[10px] font-bold bg-rose-50 text-rose-700 border border-rose-100 uppercase tracking-tighter shadow-sm">
                                Failed Delivery
                            </span>
                        </div>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @foreach($orders as $index => $order)
                            @php
                                ...
                                $vendorName = $order->vendor->name ?? 'Unknown Vendor';
                            @endphp
                            <tr class="hover:bg-rose-50/30 transition failed-order-row" data-search="{{ strtolower($order->order_no.' '.$customerName.' '.$productName.' '.$vendorName.' '.$order->status) }}">
            ...
            <div id="failedOrderNoResult" class="hidden px-6 py-16 text-center text-gray-400">
                <i class="fas fa-search text-3xl mb-3 block text-gray-300"></i>
                No matching failed delivery orders found
            </div>

            <div class="px-4 sm:px-6 py-4 border-t border-gray-100" id="failedOrderPagination">
                {{ $orders->links() }}
            </div>
        @else
            <div class="px-6 py-16 text-center text-gray-400" id="failedOrderEmptyState">
                <i class="fas fa-exclamation-triangle text-3xl mb-3 block text-gray-300"></i>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        const searchInput = document.getElementById('failedOrderSearch');
        if (!searchInput) return;

        const rows = Array.from(document.querySelectorAll('.failed-order-row'));
        const cards = Array.from(document.querySelectorAll('.failed-order-card'));
        const pagination = document.getElementById('failedOrderPagination');
        const noResultState = document.getElementById('failedOrderNoResult');
