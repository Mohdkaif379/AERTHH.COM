...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-xl border border-cyan-100/50 shadow-sm overflow-hidden">
        <div class="px-6 py-4 bg-gradient-to-r from-cyan-50 to-emerald-50 border-b border-cyan-100/50">
            <h3 class="text-sm font-semibold text-gray-700">Add New SubSubCategory</h3>
        </div>

        <form action="{{ route('subsubcategory.store') }}" method="POST" class="p-6 space-y-5">
            @csrf
...
{{-- Optional JavaScript to filter subcategories based on selected category --}}
@push('scripts')
<script>
document.getElementById('category_id')?.addEventListener('change', function() {
    ...
    // You can add AJAX here to filter subcategories based on selected category
    // For now, we'll just show all subcategories
