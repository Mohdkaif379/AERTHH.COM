...
<div class="h-[calc(100vh-140px)] flex flex-col md:flex-row bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
    <div class="w-full md:w-1/3 flex flex-col border-r border-slate-100 bg-slate-50/50">
        ...
        <div id="historyChatsList" class="flex-1 overflow-y-auto w-full p-2 space-y-1">
    ...
    <div class="w-full md:w-2/3 flex flex-col bg-white" id="historyChatArea">
        @if($completedChats->isEmpty())
            <div class="flex-1 flex flex-col items-center justify-center bg-slate-50/50 text-slate-400">
        ...
        @else
            <div class="h-16 border-b border-slate-100 px-6 flex items-center justify-between shrink-0 bg-white">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-full bg-gradient-to-br from-emerald-400 to-teal-500 text-white flex items-center justify-center font-bold text-sm shadow-sm" id="historyHeaderInitials">
                        -
                    </div>
                    <div>
                        <h3 class="font-semibold text-slate-800 text-sm" id="historyHeaderName">Select a customer</h3>
                        <p class="text-xs text-slate-400" id="historyHeaderMeta">Completed chat thread</p>
                    </div>
                </div>
            </div>

            <div id="historyEmptyState" class="flex-1 flex flex-col items-center justify-center bg-slate-50/50 text-slate-400">
                <div class="w-24 h-24 bg-sky-50 rounded-full flex items-center justify-center mb-4">
                    <i class="fa-regular fa-comments text-4xl text-sky-400"></i>
                </div>
                        ...
                        <div class="space-y-3">
                            @foreach($chat['messages'] as $message)
                                <div class="flex {{ $message->sender_type === 'support' ? 'justify-end' : 'justify-start' }}">
                                    <div class="flex flex-col {{ $message->sender_type === 'support' ? 'items-end' : 'items-start' }} w-fit max-w-[60%] md:max-w-[45%]">
