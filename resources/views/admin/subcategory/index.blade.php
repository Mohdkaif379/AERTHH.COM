...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h3 class="text-sm font-medium text-gray-700">All SubCategories</h3>
            <p class="text-[10px] text-gray-400">Manage your product subcategories</p>
        </div>
    ...
    <div class="bg-white rounded-xl border border-cyan-100/50 shadow-sm overflow-hidden">
        <div class="px-6 py-4 bg-gradient-to-r from-cyan-50 to-emerald-50 border-b border-cyan-100/50">
            <div class="flex items-center justify-between">
                <h3 class="text-sm font-semibold text-gray-700">SubCategories List</h3>
            ...
            <table class="w-full text-xs">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-cyan-50/30 transition-colors duration-200">
                        ...
                        {{-- Name --}}
                        <td class="px-4 py-3 font-medium text-gray-700">{{ $subcategory->name }}</td>
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center space-x-2">
                                ...
                                <button type="button"
                                   onclick="openDeleteModal({{ $subcategory->id }}, '{{ $subcategory->name }}')"
                    ...
                    <tr>
                        <td colspan="6" class="px-4 py-8 text-center text-gray-400 text-xs">
                            ...
                            <a href="{{ route('subcategory.create') }}" class="block mt-2 text-cyan-500 hover:text-cyan-600">
        ...
        <div class="px-6 py-3 bg-gray-50 border-t border-gray-100 flex items-center justify-between">
            <p class="text-[10px] text-gray-400">
                Showing {{ $subcategories->count() }} {{ Str::plural('entry', $subcategories->count()) }}
            </p>
        </div>
    </div>
</div>
...
document.getElementById('search-input')?.addEventListener('keyup', function() {
    ...
    rows.forEach(row => {
        ...
        if (name && category) {
            let matches = name.includes(search) || category.includes(search);
...
function openDeleteModal(itemId, itemName) {
    ...
    deleteButton.href = `{{ url('admin/subcategory/delete') }}/${itemId}`;
