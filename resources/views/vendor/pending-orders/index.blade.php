...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Pending Orders</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Review and approve pending orders ({{ $orders->total() ?? 0 }})</p>
        </div>
        <div class="flex items-center gap-2">
            <button onclick="exportPendingOrders()" class="inline-flex items-center gap-2 bg-orange-500 hover:bg-orange-600 text-white px-5 py-2.5 rounded-lg font-medium shadow-sm hover:shadow transition-all duration-200">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            <div class="relative">
                ...
                <input type="text" id="searchInput" placeholder="Search by Order ID, Customer..."
                    class="w-full pl-9 pr-4 py-2 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 focus:ring-2 focus:ring-orange-500/50">
            </div>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden shadow-sm">
        ...
        <div id="noResults" class="hidden flex items-center justify-center py-12 text-gray-500 dark:text-gray-400">
            <i class="fa fa-inbox text-3xl mr-3 opacity-50"></i>
            <span>No pending orders found</span>
        </div>
        ...
        <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
            ...
            <div class="p-6">
                ...
                <div class="flex gap-3">
                    ...
                    <button id="confirmBtn" onclick="confirmAction()" class="flex-1 px-4 py-2.5 bg-orange-500 hover:bg-orange-600 text-white rounded-xl transition-all font-medium shadow-lg hover:shadow-xl">
                        Confirm
                    </button>
                </div>
            </div>
...
<script>
        ...
        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/40 transition">
          ...
          <td class="px-4 py-3 whitespace-nowrap">
            <div class="flex items-center gap-1.5">
              ...
              <button onclick="quickConfirm(${order.id}, '${order.status}')" class="p-1.5 text-green-600 hover:bg-green-50 dark:hover:bg-green-950/30 rounded-md transition" title="Confirm">
                <i class="fa fa-check text-sm"></i>
              </button>
              <button onclick="quickReject(${order.id})" class="p-1.5 text-red-600 hover:bg-red-50 dark:hover:bg-red-950/30 rounded-md transition" title="Reject">
    ...
    window.quickConfirm = function(id, status) {
        if (status !== 'pending') return;
        currentOrderId = id;
        currentAction = 'confirm';
        document.getElementById('modalTitle').innerHTML = '<i class="fa fa-check-circle text-green-500"></i> Confirm Order';
        document.getElementById('modalMessage').textContent = 'Approve this order for processing?';
        document.getElementById('confirmBtn').textContent = 'Confirm Order';
        document.getElementById('confirmBtn').className = 'flex-1 px-4 py-2.5 bg-green-500 hover:bg-green-600 text-white rounded-xl transition-all font-medium shadow-lg hover:shadow-xl';
    ...
    window.quickReject = function(id) {
        ...
        document.getElementById('modalTitle').innerHTML = '<i class="fa fa-times-circle text-red-500"></i> Reject Order';
        document.getElementById('modalMessage').textContent = 'Reject this order? Customer will be notified.';
        document.getElementById('confirmBtn').textContent = 'Reject Order';
...
window.confirmAction = function() {
  ...
  form.method = 'POST';
  form.action = `/vendor/pending-orders/${currentOrderId}/update-status`;
  ...
  statusInput.value = currentAction === 'confirm' ? 'confirmed' : 'rejected';
  ...
  form.submit();
    ...
    window.exportPendingOrders = function() {
        window.open('/vendor/pending-orders/export', '_blank');
    };
