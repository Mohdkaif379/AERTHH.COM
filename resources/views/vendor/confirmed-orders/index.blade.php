...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Confirmed Orders</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Manage confirmed orders ({{ $orders->total() ?? 0 }})</p>
        </div>
        <div class="flex items-center gap-2">
            <button onclick="exportConfirmedOrders()" class="inline-flex items-center gap-2 bg-orange-500 hover:bg-orange-600 text-white px-5 py-2.5 rounded-lg font-medium shadow-sm hover:shadow transition-all duration-200">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            <div class="relative">
                ...
                <input type="text" id="searchInput" placeholder="Search by Order ID, Customer..."
                    class="w-full pl-9 pr-4 py-2 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 focus:ring-2 focus:ring-green-500/50">
            </div>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden shadow-sm">
        ...
        <div id="noResults" class="hidden flex items-center justify-center py-12 text-gray-500 dark:text-gray-400">
            <i class="fa fa-check-circle text-3xl mr-3 opacity-50"></i>
            <span>No approved orders found</span>
        </div>
        ...
        <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
            ...
            <div class="p-6">
                ...
                <div class="flex gap-3">
                    ...
                    <button id="confirmBtn" onclick="confirmAction()" class="flex-1 px-4 py-2.5 bg-green-500 hover:bg-green-600 text-white rounded-xl transition-all font-medium shadow-lg hover:shadow-xl">
                        Confirm
                    </button>
                </div>
            </div>
...
<script>
        ...
        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/40 transition">
          ...
          <td class="px-4 py-3 whitespace-nowrap">
            <div class="flex items-center gap-1.5">
              ...
              <button onclick="quickPackaging(${order.id}, '${order.status}')" class="p-1.5 text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-950/30 rounded-md transition" title="Packaging">
                <i class="fa fa-box text-sm"></i>
              </button>
    ...
    window.quickPackaging = function(id, status) {
        if (status !== 'confirmed') return;
        currentOrderId = id;
        currentAction = 'packaging';
        document.getElementById('modalTitle').innerHTML = '<i class="fa fa-box text-blue-500"></i> Packaging';
        document.getElementById('modalMessage').textContent = 'Move this order to packaging stage?';
        document.getElementById('confirmBtn').textContent = 'Packaging';
        document.getElementById('confirmBtn').className = 'flex-1 px-4 py-2.5 bg-blue-500 hover:bg-blue-600 text-white rounded-xl transition-all font-medium shadow-lg hover:shadow-xl';
  ...
  form.method = 'POST';
  form.action = `/vendor/confirmed-orders/${currentOrderId}/update-status`;
  ...
  statusInput.value = currentAction === 'packaging' ? 'packaging' : 'cancelled';
  ...
  form.submit();
    ...
    window.exportApprovedOrders = function() {
        window.open('/vendor/confirmed-orders/export', '_blank');
    };
