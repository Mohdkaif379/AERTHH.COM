...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <div>
      <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Rejected Products</h1>
      <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Products that have been rejected by the admin</p>
    </div>
  ...
  <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-4 mb-6 shadow-sm">
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
      ...
      <select id="categoryFilter" class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800">
        ...
        @php
          $categories = collect($rejectedProducts)->pluck('category.name')->unique()->filter();
        @endphp
      ...
      <select id="brandFilter" class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800">
        ...
        @php
          $brands = collect($rejectedProducts)->pluck('brand.name')->unique()->filter();
        @endphp
  ...
  <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden shadow-sm">
    ...
    <div id="noResults" class="hidden flex flex-col items-center justify-center py-12 text-gray-500 dark:text-gray-400">
      ...
      <p>No rejected products found matching your filters.</p>
    </div>
  </div>
</div>

<script>
const rejectedProducts = @json($rejectedProducts ?? []);
  ...
  const filtered = rejectedProducts.filter((p) => {
      ...
      return `<tr class="hover:bg-gray-50 dark:hover:bg-gray-800/40 transition duration-150">
          ...
          <td class="px-4 py-3 whitespace-nowrap">
            <div class="flex items-center gap-1.5">
                <a href="/vendor/products/${product.id}/edit" class="p-1.5 text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-950/30 rounded-md transition" title="Edit"><i class="fa fa-pen text-sm"></i></a>
            </div>
          </td>
        </tr>`;
    }).join('');
  ...
  if (rejectedProducts && rejectedProducts.length) {
