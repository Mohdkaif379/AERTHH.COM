...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Packaging Orders</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Prepare and ship packaging orders ({{ $orders->total() ?? 0 }})</p>
        </div>
        <div class="flex items-center gap-2">
            <button onclick="exportPackagingOrders()" class="inline-flex items-center gap-2 bg-orange-500 hover:bg-orange-600 text-white px-5 py-2.5 rounded-lg font-medium shadow-sm hover:shadow transition-all duration-200">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            <div class="relative">
                ...
                <input type="text" id="searchInput" placeholder="Search by Order ID, Customer..."
                    class="w-full pl-9 pr-4 py-2 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 focus:ring-2 focus:ring-blue-500/50">
            </div>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden shadow-sm">
        ...
        <div id="loadingState" class="flex items-center justify-center py-12 text-gray-500 dark:text-gray-400">
            ...
            <span>Loading packaging orders...</span>
        </div>
        <div id="noResults" class="hidden flex items-center justify-center py-12 text-gray-500 dark:text-gray-400">
            <i class="fa fa-box text-3xl mr-3 opacity-50"></i>
            <span>No packaging orders found</span>
        </div>
        ...
        <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
            <div class="p-6 border-b border-gray-200 dark:border-gray-800">
                <h3 id="modalTitle" class="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
                    <i class="fa fa-question-circle text-blue-500"></i>
                    Confirm Action
                </h3>
            </div>
            <div class="p-6">
                ...
                <div class="flex gap-3">
                    ...
                    <button id="confirmBtn" onclick="confirmAction()" class="flex-1 px-4 py-2.5 bg-blue-500 hover:bg-blue-600 text-white rounded-xl transition-all font-medium shadow-lg hover:shadow-xl">
                        Confirm
                    </button>
                </div>
            </div>
        ...
        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/40 transition">
          ...
          <td class="px-4 py-3 whitespace-nowrap">
            <div class="flex items-center gap-1.5">
              ...
              <button onclick="quickOutForDelivery(${order.id}, '${order.status}')" class="p-1.5 text-purple-600 hover:bg-purple-50 dark:hover:bg-purple-950/30 rounded-md transition" title="Out for Delivery">
                <i class="fa fa-truck text-sm"></i>
              </button>

              <button onclick="quickCancel(${order.id})" class="p-1.5 text-red-600 hover:bg-red-50 dark:hover:bg-red-950/30 rounded-md transition" title="Cancel">
...
window.quickOutForDelivery = function(id, status) {
        if (status !== 'packaging') return;
        currentOrderId = id;
        currentAction = 'out_for_delivery';
        document.getElementById('modalTitle').innerHTML = '<i class="fa fa-truck text-purple-500"></i> Mark as Out for Delivery';
        document.getElementById('modalMessage').textContent = 'Mark this order as out for delivery?';
        document.getElementById('confirmBtn').textContent = 'Out for Delivery';
        document.getElementById('confirmBtn').className = 'flex-1 px-4 py-2.5 bg-purple-500 hover:bg-purple-600 text-white rounded-xl transition-all font-medium shadow-lg hover:shadow-xl';
        document.getElementById('quickActionModal').classList.remove('hidden');
    };


    window.quickCancel = function(id) {
        currentOrderId = id;
        currentAction = 'cancelled';
        document.getElementById('modalTitle').innerHTML = '<i class="fa fa-times-circle text-red-500"></i> Cancel Order';
        document.getElementById('modalMessage').textContent = 'Cancel this order? Customer will be notified.';
        document.getElementById('confirmBtn').textContent = 'Cancel Order';
        document.getElementById('confirmBtn').className = 'flex-1 px-4 py-2.5 bg-red-500 hover:bg-red-600 text-white rounded-xl transition-all font-medium shadow-lg hover:shadow-xl';
        document.getElementById('quickActionModal').classList.remove('hidden');
    };
  ...
  form.method = 'POST';
  form.action = `/vendor/packaging-orders/${currentOrderId}/update-status`;
...
statusInput.value = currentAction === 'out_for_delivery' ? 'out_for_delivery' : 'cancelled';
  ...
  form.submit();
    ...
    window.exportPackagingOrders = function() {
        window.open('/vendor/packaging-orders/export', '_blank');
    };
