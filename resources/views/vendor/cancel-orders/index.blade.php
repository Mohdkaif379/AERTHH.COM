...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Cancelled Orders</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Review cancelled orders ({{ $orders->total() ?? 0 }})</p>
        </div>
        <div class="flex items-center gap-2">
            <button onclick="exportCancelledOrders()" class="inline-flex items-center gap-2 bg-orange-500 hover:bg-orange-600 text-white px-5 py-2.5 rounded-lg font-medium shadow-sm hover:shadow transition-all duration-200">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            <div class="relative">
                ...
                <input type="text" id="searchInput" placeholder="Search by Order ID, Customer, Product..."
                    class="w-full pl-9 pr-4 py-2 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 focus:ring-2 focus:ring-red-500/50 focus:border-transparent">
            </div>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden shadow-sm">
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                <thead class="bg-gray-50 dark:bg-gray-800/60">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider">Reason</th>
        ...
        <div id="noResults" class="hidden flex items-center justify-center py-12 text-gray-500 dark:text-gray-400">
            <i class="fa fa-times-circle text-3xl mr-3 opacity-50 text-red-500"></i>
...
<script>
    ...
    function getCancelBadge(reason) {
        return `<span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800 dark:bg-red-900/40 dark:text-red-300">
            ${reason || 'N/A'}
        </span>`;
    }
    ...
    function renderOrders() {
        ...
        filteredOrders = orders.data.filter(order => {
            ...
            const dateMatch = !dateFilter || createdAt.toDateString() === new Date(dateFilter).toDateString();
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/40 transition-colors">
                    ...
                    <td class="px-4 py-3">
                        <div class="text-sm font-medium text-gray-900 dark:text-white">
                            ${order.customer ? [order.customer.first_name, order.customer.last_name].filter(Boolean).join(' ') : 'N/A'}
                        </div>
                    ...
                    <td class="px-4 py-3 whitespace-nowrap">
                        ${getCancelBadge((order.cancel_reason || '').toLowerCase())}
                    </td>
    ...
    window.printInvoice = function(id) {
        window.open(`/vendor/cancel-orders/${id}/invoice`, '_blank');
    };

    window.exportCancelledOrders = function() {
        window.open('/vendor/cancel-orders/export', '_blank');
    };
