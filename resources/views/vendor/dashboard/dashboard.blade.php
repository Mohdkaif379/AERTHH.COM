...
<div class="min-h-screen  dark:from-slate-900 dark:to-slate-800 p-2">
    ...
    <div class="mb-6">
        <h2 class="text-2xl font-semibold text-gray-900 dark:text-white">
            Welcome back, 
            <span class="text-orange-500" id="welcomeVendorName">{{ $vendor['name'] ?? 'Vendor' }}</span>
        </h2>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-5 gap-6 mb-8">
        ...
        <div class="lg:col-span-2 bg-white/70 dark:bg-slate-800/70 backdrop-blur-xl rounded-3xl p-6 border shadow-2xl">
            <div class="flex items-center gap-4 mb-4">
                <img src="{{ $vendor['image'] ?? 'https://ui-avatars.com/api/?name=' . urlencode($vendor['name']) . '&size=64' }}" 
                     alt="{{ $vendor['name'] }}" class="w-16 h-16 rounded-xl ring-2 ring-emerald-400/50">

                <div>
                    <h3 class="font-bold text-lg">{{ $vendor['name'] }}</h3>
                    <p class="text-xs text-gray-500">{{ $vendor['email'] }}</p>
                    @if(($vendor['status'] ?? 0) == 1)
                    <span class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs bg-emerald-100 text-emerald-800 font-medium mt-1">
                        <i class="fa-solid fa-check-circle text-emerald-500"></i>
                        Verified
                    </span>
                    @else
                    <span class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs bg-orange-100 text-orange-800 font-medium mt-1">
            ...
            <div class="grid grid-cols-3 gap-2 text-xs">
                <div class="text-center p-2 bg-emerald-50 rounded-lg">
                    <div class="font-bold text-emerald-600">{{ number_format($revenue ?? 0, 0) }}</div>
                    <div>Revenue</div>
                </div>
                <div class="text-center p-2 bg-purple-50 rounded-lg">
                    <div class="font-bold text-purple-600">{{ number_format($profit ?? 0, 0) }}</div>
                    <div>Profit</div>
                </div>
                <div class="text-center p-2 bg-blue-50 rounded-lg">
                    <div class="font-bold text-blue-600">{{ \App\Models\Order::where('vendor_id', $vendor['id'])->count() ?? 0 }}</div>
                    <div>Orders</div>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    new Chart(document.getElementById('revenueChart').getContext('2d'), {
        type: 'line',
        data: { labels: {!! json_encode($months) !!}, datasets: [{ label: 'Revenue', data: {!! json_encode($revenues) !!}, borderColor: '#10b981', fill: true, tension: 0.4 }] },
    ...
    new Chart(document.getElementById('profitChart').getContext('2d'), {
        type: 'line',
        data: { labels: {!! json_encode($months) !!}, datasets: [{ label: 'Profit', data: {!! json_encode($profits) !!}, borderColor: '#8b5cf6', fill: false, tension: 0.4 }] },
        options: { responsive: true, scales: { y: { beginAtZero: true } } }
    });
});
@endif
...
new Chart(statusCtx, {
    ...
    data: {
        ...
        datasets: [{ data: {!! json_encode(array_values($statusCounts->toArray())) !!}, backgroundColor: ['#f59e0b', '#10b981', '#3b82f6', '#ef4444', '#8b5cf6'] }]
    },
